...
In general, for n variables, there will be 2^n possible assignments, so the approach of "trying everything" is not efficient for large n. In fact, this is the SAT ("satisfiability") problem, 
which is NP-complete, meaning that it is "one of the hardest problems to which solutions can be verified quickly. 
...
SMT stands for SAT modulo theories; it generalizes SAT to formulas involving integers, strings, arrays, and so on 
(the details of how it can do this are interesting but complicated, and we probably wont be able to get to them in class). For example, we can use Z3 to find a solution for the system of equations 3x - 2y = 1, y - x = 1.
"""
...
"""
One of the most prominent uses of SMT in industry (especially now when so much software is being produced with little
 else to ensure correctness) is in software verification. You can see how we can prove interesting properties about
...
"""
Often, SMT solvers are not used to find a solution, but to prove that no solution exists. For instance, say that we want 
to prove the mathematical fact y > 0 ==> x + y > x. How might we do this using z3?
"""
...
def proof_by_unsat():
    ...
    match s.check():
        case z3.unsat:
            print(
                "The formula is UNSAT, meaning no contradiction can be found. Therefore the original formula must always be true.  QED.")
...
def demorgans_proof():
    ...
    def prove(f):
        ...
        otherwise print "The formula f is false, with counterexample given by: " and the model that shows the formula to be false.
        """
        # TODO: YOUR CODE HERE
        s = Solver()
        # see if counterexample is true
        s.add(Not(f))

        match s.check():
            case z3.unsat:
                print("No conterexample can be found, therefore the statement is true:", f)
            case z3.sat:
...
"""
Let us try to use z3 to solve a classic kind of puzzle. You will need to come up with an encoding of the 
relevant part of the puzzle into variables and constraints as well translate the output of the solver into a solution.
"""
...
def wedding_planning():
    ...
    # between left middle and right
    s.add(alice >= 0, alice <= 2)
    s.add(bob >= 0, bob <= 2)
    s.add(charlie >= 0, charlie <= 2)

    # can't equal one another
    s.add(And(alice != bob, bob != charlie, alice != charlie))

    # alice cannot sit next to charlie
    s.add(And(alice + 1 != charlie, alice - 1 != charlie))
    # alice cannot sit in the leftmost chair
    s.add(alice != 0)

    # bob cannot sit to the right of charlie
    s.add(bob < charlie)

    match s.check():
        case z3.sat:
            print(f"Alice sits on the left, Charlie in the middle, and Bob on the right. {s.model()}")
        case z3.unsat:
...
In case you do not know, sudoku is a puzzle where the goal is to insert numbers in boxes to satisfy the following condition: 
each row, column, and 3x3 box must contain the digits 1 through 9 exactly once.

You will need to transform an arbitrary sudoku puzzle template (where zero represents the unfilled boxes) into a solved
 puzzle (or show that the puzzle is impossible).
"""
...
def sudoku(puzzle):
    ...
    """
    # TODO: YOUR CODE HERE
    grid = [[Int(f'cell{r}_{c}') for c in range(9)] for r in range(9)]
    ...
    # Distinct columns
    for c in range(9):
        s.add(Distinct(grid[0][c], grid[1][c], grid[2][c], grid[3][c], grid[4][c],
                       grid[5][c], grid[6][c], grid[7][c], grid[8][c]))

    # Distinct boxes
    for r in range(0, 9, 3):
        for c in range(0, 9, 3):
            s.add(Distinct(grid[r][c], grid[r][c + 1], grid[r][c + 2],
    ...
    match s.check():
        case z3.sat:
            m = s.model()
            solved_grid = [[m[grid[r][c]].as_long() for c in range(9)] for r in range(9)]
...
"""
The final puzzle you will solve using Z3 is the Coin Sum problem. Here in the US we have (or used to have) pennies 
worth 1 cent, nickels worth 5 cents, dimes worth 10 cents, quarters worth 25 cents, half dollar coins worth 50 cents,
and dollar coins worth 100 cents.
